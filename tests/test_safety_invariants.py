...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=False,
    ), patch("bongo.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("bongo.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "bongo.runtime"

    with patch("bongo.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
