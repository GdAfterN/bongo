...
class WorkspaceContext:
    ...
    def build(cls, cwd, repo_root_override=None):
        ...
        return cls(
            ...
            branch=git(["branch", "--show-current"], "-") or "-",  # 获取当前分支
            default_branch=(
                lambda branch: branch[len("origin/") :] if branch.startswith("origin/") else branch  # 获取主分支
            )(git(["symbolic-ref", "--short", "refs/remotes/origin/HEAD"], "origin/main") or "origin/main"),
            status=clip(git(["status", "--short"], "clean") or "clean", 1500),  # 最近哪些文件有变动但未提交
            recent_commits=[line for line in git(["log", "--oneline", "-5"]).splitlines() if line],  #最近提交了哪些改动
            project_docs=docs,  # 根据预设的白名单列表里的白名单文件名，去当前目录和子目录中找到的高价值文档
        )
