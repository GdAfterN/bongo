...
class ContextManager:
    ...
    def _render_sections_without_reduction(self, section_texts, selected_notes=None):
        # 如果没有传入选中的笔记，初始化为空列表，防止后续处理 None 报错
        selected_notes = selected_notes or []
        # 初始化相关记忆部分的文本行，以标题开头
        relevant_lines = ["Relevant memory:"]
        # 如果存在检索到的相关笔记
        if selected_notes:
            # 将每条笔记的文本格式化为列表项（"- 内容"）并添加到行列表中
            relevant_lines.extend(f"- {note['text']}" for note in selected_notes)
        else:
            # 如果没有相关笔记，明确标记为 "none"，让模型知道此处无额外上下文
            relevant_lines.append("- none")
        # 将所有行用换行符拼接成完整的“相关记忆”原始字符串
        relevant_raw = "\n".join(relevant_lines)
        # 从 agent 的 session 中安全地提取对话历史记录，默认为空列表
        history = list(getattr(self.agent, "session", {}).get("history", []))
        # 将结构化的历史记录列表转换为纯文本字符串
        history_raw = self._raw_history_text(history)
        # 返回各个 Section 的渲染对象字典。在不压缩模式下，raw（原始内容）与 rendered（最终内容）一致
        return {
            # 系统前缀：直接包装，不改变内容
            ...
            # 当前请求：用户本次的输入，优先级最高，budget 设为 0 表示不参与预算压缩分配
